...
bench
	...
	.add("complex: mixed operators", () => {
		parse('email? AND verified = true AND status : ["active", "premium"] AND user.age >= 18 AND NOT suspended');
	})
	.add("complex: permission check", () => {
		parse('(user.role = "admin" AND permissions.write) OR (user.role = "moderator" AND permissions.moderate)');
	});
...
bench
	...
	.add("api: date range with status", () => {
		parse('createdAt >= "2024-01-01" AND createdAt <= "2024-12-31" AND status : ["active", "pending"]');
	})
