...
describe("SQL", () => {
	...
	describe("DuckDB Support", () => {
		test("numbered parameters (DuckDB/PostgreSQL style)", () => {
			const ast: ASTNode = {
				type: "and",
				left: {
					type: "comparison",
		...
		test("question mark parameters (DuckDB/MySQL/SQLite style)", () => {
			const ast: ASTNode = {
				type: "and",
				left: {
					type: "comparison",
			...
			const result = toSQL(ast, { parameterStyle: "question" });
			expect(result.sql).toBe("(price <= ? AND category LIKE ?)");
			expect(result.params).toEqual([1000, "laptop"]);
		});

		test("nested field access with field mapper", () => {
			const ast: ASTNode = {
				type: "comparison",
		...
		test("complex query with IN clause for analytics", () => {
			const ast: ASTNode = {
				type: "and",
				left: {
					type: "oneOf",
			...
			const result = toSQL(ast, { parameterStyle: "numbered" });
			expect(result.sql).toBe("(region IN ($1, $2, $3) AND revenue > $4)");
			expect(result.params).toEqual(["US", "EU", "APAC", 10000]);
		});

		test("parquet file querying pattern", () => {
			const ast: ASTNode = {
				type: "and",
				left: {
					type: "comparison",
					field: "timestamp",
					operator: ">=",
					value: { type: "string", value: "2024-01-01" },
				},
				right: {
					type: "comparison",
					field: "status_code",
			...
			expect(result.sql).toBe('("timestamp" >= $1 AND "status_code" < $2)');
			expect(result.params).toEqual(["2024-01-01", 400]);
		});
	});
});
